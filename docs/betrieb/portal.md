...
**Diese Dateien müssen vor dem ersten `docker compose up` in `portal/data/zertifikate/` liegen**
...
- **Client-Zertifikate** (`portal-kanal.crt`) laufen nach einem Jahr ab. Der obige Befehl erneut
  ...
  Danach `ca.crt` nicht erneut kopieren (unverändert); nur sicherstellen, dass das Hauptsystem den
  Kanal neu aufbaut (automatisch beim nächsten Long-Poll-Zyklus, notfalls Hauptsystem neu starten).
- **Schlüsselrotation** (z. B. bei Verdacht auf Kompromittierung): vor dem erneuten Aufruf
  ...
  neues Schlüsselpaar samt Zertifikat an.
- **Ablauf der internen CA** (`ca.crt`, gültig 5 Jahre; eine Warnung erscheint im Log des
  Hauptsystems, sobald die Restlaufzeit unter einem Jahr liegt): `ca.crt` und `ca.key` gemeinsam
  entfernen und den Befehl erneut ausführen (erzeugt eine neue CA und neue Client-Zertifikate);
  anschließend die neue `ca.crt` erneut auf das Portal kopieren (Caddy `trust_pool` neu laden, z. B.
...
Die Portal-Datenbanken (`beachhub_portal`, `beachhub_portal_test`) werden immer mit
`TEMPLATE template0 ENCODING 'UTF8'` angelegt (`portal/deploy/init-test-db.sql`, CI); ein einfaches
`CREATE DATABASE` aus `template1` liefert auf manchen Systemen `SQL_ASCII`, woran psycopg3 beim
ersten Connect abstürzt.
